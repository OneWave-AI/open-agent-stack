---
name: polish-pass
description: The de-AI pass -- audit a vibe-coded app for template smell in design and copy, then fix it: editorial layout, purposeful motion, human-sounding words. Use after any build session, before anyone else sees it.
---
...
4. **Motion with intent.** Replace scattered hover effects with a small motion system: one entrance treatment, one emphasis move, consistent durations/easings. Motion that communicates hierarchy, not motion because Framer Motion was installed. Respect `prefers-reduced-motion`.
