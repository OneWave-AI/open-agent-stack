...
4. **First real screen.** Build the landing/home screen properly against the theme -- type scale, spacing, one signature motion -- so every screen after has a quality bar to match, not a placeholder to excuse.
5. **Boot it.** `npm run dev`, verify it renders, commit. The scaffold is done when it runs, not when the files exist.
